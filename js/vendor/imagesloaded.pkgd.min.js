...
( function( global, factory ) {
  ...
  ImagesLoadedPN.prototype.addImageAttrImages = function( elem ) {
    ...
    // If Electron app, translate urls to use IP
    if (app && app.options && app.options.peertubeUseIp == true && app.peertubeHandler &&
        app.peertubeHandler.helpers && typeof _Electron != 'undefined' && _Electron) {

      app.peertubeHandler.helpers.convertUrlWithIp(url).then((newUrl) => {
        var img = this.addImageAttr(newUrl, elem);
        img.applyStyle();
      });

    }
    // If browser, translate urls to use domain name
    else if (app && app.options && app.options.peertubeUseIp == true && app.peertubeHandler
             && app.peertubeHandler.helpers)  {

      app.peertubeHandler.helpers.convertIpWithUrl(url).then((newUrl) => {
        var img = this.addImageAttr(newUrl, elem);
        img.applyStyle();
      });
