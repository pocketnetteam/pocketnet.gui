...
var PeertubeRequest = function(app = {}){
    ...
    var directclear = function(url, data, signal, p){
        ...
        if(data && !_.isEmpty(data)) ps.body = serialize(data)

        return fetch(url, ps).then(r => {
...
PeerTubePocketnet = function(app){
    ...
    var transcode = async function(file){
        ...
        ///test
        await ffmpeg.run('-i', name, '-acodec', 'copy', '-vcodec', 'h264', '-s', '720x1280', name + 'output.mp4');
    ...
    var methods = {
        ...
        video : {
            path : function({id}){ return 'api/v1/videos/' + id }
        },
    ...
    var request = function(method, data, options){
        ...
        return self.api.proxy.bestIfNeed(options.host && !options.best ? false : true).then(host => {
            ...
            return self.api.user.authIfNeed(meta.authorization, options.host, meta.renew)
            ...
            if (meta.headers){
                requestoptions.headers = _.extend(requestoptions.headers, meta.headers)
            }
            ...
            if (meta.formdata || meta.axios){
                ...
                return axios[(requestoptions.method || "post").toLowerCase()](
                    'https://' + options.host +'/'+ meta.path, data, axiosoptions).then(r => {
                    return r.data || {}
                }).catch((e)=> {
            ...
            return proxyRequest.fetch('https://' + options.host, meta.path, data, requestoptions)
    ...
    self.api = {
        ...
        proxy : {
            ...
            best : function(){
                ...
                return app.api.fetch('peertube/best').then((data) => {
        ...
        videos : {
            ...
            update : function(url, parameters = {}, options = {}){
                ...
                if (parameters.image){
                    data.thumbnailfile = data.previewfile = dataURLtoFile(parameters.image.data)
                }
            ...
            upload : function(parameters, options){
                ...
                return self.api.videos.checkQuota(parameters.video.size).then((rme) => {
                    ...
                    if (parameters.image){
                        data.thumbnailfile = data.previewfile = dataURLtoFile(parameters.image.data, parameters.image.name)
                    }

                    return request('uploadVideo', data, options).then(r => {
                        if(!r.video) return Promise.reject(error('uploaderror'))

                        return Promise.resolve(self.composeLink(options.host, r.video.uuid))
                    }).catch(e => {
            ...
            checkQuota : function(size){
                return self.api.user.me().then(rme => {
                    ...
                    return self.api.videos.quota().then(rqu => {
                        ...
                        if(
                            ((size + rqu.videoQuotaUsedDaily < rme.videoQuotaDaily + VIDEO_QUOTA_CORRECTION) || rme.videoQuotaDaily < 0) &&
                            ((size + rqu.videoQuotaUsed < rme.videoQuota + VIDEO_QUOTA_CORRECTION) || rme.videoQuota < 0)
                            ){
        ...
        user : {
            ...
            me : function(){
                return request('me').then(r => {
                    ...
                    if(!data.channelId || !data.videoQuotaDaily) return Promise.reject(error('usersMe'))
            ...
            auth : function(host, renew){
                ...
                return request('oauthClientsLocal').then(({client_id, client_secret}) => {
            ...
            getToken : function(data){
                ...
                return request('getToken', data, options).then(({access_token, refresh_token}) => {
