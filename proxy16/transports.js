...
class WrappedAxios {
    ...
    static handleError(error) {
        const isConnRefused = error.message.includes('ECONNREFUSED 127.0.0.1:9050');
...
class Transports {
    ...
    getTorAgent() {
        if (!this.torAgent) {
            const url = new URL('socks5h://127.0.0.1:9050');
    ...
    isTorRefuseConnections(error) {
        return error.message.includes('ECONNREFUSED 127.0.0.1:9050');
    }
}
