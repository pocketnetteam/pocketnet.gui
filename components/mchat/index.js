	...
	var Essense = function(p){
		...
		var events = {
			...
			resizeWindow : function(){
				if (topcaption){
					topcaption.setOffset([$('#menu .menuWrapper').height(), 0]);
			...
			if (essenseData.view == 'buildin')
			{
				...
				if(!isMobile())
					...
					topcaption = new Caption({
						...
						offset: [$('#menu .menuWrapper').height(), 0],
		...
		return {
			...
			getdata : function(clbk, p){
				...
				chat = deep(p, 'settings.essenseData.chat');

				
				if (chat) 
				{

					self.app.platform.sdk.chats.info([chat], function(d){
...
if(typeof module != "undefined")
{
	...
	app.modules.mchat = {};
	app.modules.mchat.module = mchat;
