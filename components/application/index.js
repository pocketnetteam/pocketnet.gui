var application = (function () {
  ...
  var Essense = function (p) {
    ...
    var calculateRatingStats = function (scores) {
      ...
      var createDistributionItem = function (stars, index) {
        return {
          ...
          percentage:
            total > 0 ? Math.round((distribution[index] / total) * 100) : 0,
        };
      };
      ...
      return comments
        ...
        .map(function (comment) {
          ...
          return {
            ...
            date: comment.time
              ? new Date(comment.time).toLocaleDateString("ru-RU", {
                  day: "numeric",
                  month: "short",
                  year: "numeric",
                })
    ...
    var actions = {
      ...
      translate: function (id, el) {
        var comment = comments.find((c) => c.id === id);
        if (!comment) return;

        var lang = "en";
        if (self.app.localization.key) lang = self.app.localization.key;

        el.find(".reviewText").addClass("translating");
				console.log(comment.id, comment)
        self.app.platform.sdk.translate.comment
          .request(comment.id, lang)
          .then((r) => {
            ...
            if (r) {
              if (r.message) m = r.message;
                ...
                try {
                  var p = JSON.parse(r.m);
                  if (p.message) m = p.message;
                } catch (e) {}
              }
            }

            el.find(".reviewText").removeClass("translating");
            el.find(".reviewText").html(m);
            el.find(".translateButton").remove();
          })
          .catch((e) => {
            console.error(e);
      ...
      loadcomments: function (callback) {
        ...
        self.app.platform.sdk.comments.getclear(
          application.hash,
          "",
          function (loadedComments, error) {
            if (error) {
      ...
      install: function () {
        ...
        self.app.apps
          ...
          .catch((e) => {
            console.error(e);
            sitemessage(
              self.app.localization.e("miniApp_installErrorMessage"),
              null,
              5000
            );
          })
      ...
      showRatingPrompt: function () {
        ...
        var openCount = parseInt(
          localStorage["app_open_count_" + appId] || "0"
        );
        openCount++;
        localStorage["app_open_count_" + appId] = openCount.toString();

        var cancelCount = parseInt(
          localStorage["app_rating_cancel_count_" + appId] || "0"
        );
        ...
        new dialog({
          ...
          fail: function () {
            var currentCancelCount = parseInt(
              localStorage["app_rating_cancel_count_" + appId] || "0"
            );
            localStorage["app_rating_cancel_count_" + appId] = (
    ...
    var events = {
      ...
      loaded: function (p) {
        ...
        setTimeout(() => {
          if (el.c) el.c.find(".captionRow").addClass("notactive");
        }, 2000);
      },
      ...
      chats: {
        ...
        init: function (el) {
          ...
          self.app.platform.matrixchat.clbks.ALL_NOTIFICATIONS_COUNT.application =
            function (count) {
              setH(count);
            };
    ...
    var renders = {
        ...
        self.shell(
          ...
          function (p) {
            ...
            p.el.find(".translateButton").on("click", function () {
              ...
              actions.translate(commentId, reviewEl);
            });
          ...
          function (p) {
            ...
            p.el.find(".starRatingInput i").on("click", function () {
              ...
              p.el.find(".starRatingInput i").each(function () {
                ...
                $(this)
                  .removeClass("fas far")
                  .addClass(starRating <= selectedRating ? "fas" : "far");
              });

              p.el
                .find(".ratingLabel")
                .text(
                  self.app.localization.e("yourRating") +
                    ": " +
                    selectedRating +
                    " " +
      ...
      frameremote: function (scope, clbk) {
        ...
        var src = self.app.apps.normalizeScopeUrl(
          _scope + "/" + (actions.getpath() || application.manifest.start || "")
        );
        curpath = actions.getpath();
        ...
        self.shell(
          ...
          function (p) {
            ...
            p.el.find(".refresh").on("click", () => {
              ...
              if (electron) electron.ipcRenderer.send("electron-refresh");
            });
    ...
    return {
        ...
        self.app.apps.get
          ...
          .then(async (f) => {
            ...
            if (f) {
              ...
              appdata = f.appdata?.data;
              grantedPermissions = f.appdata?.permissions?.filter(
      ...
      destroy: function () {
        ...
        delete self.app.platform.matrixchat.clbks.ALL_NOTIFICATIONS_COUNT
          .application;
      },
